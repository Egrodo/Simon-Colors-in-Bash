...
score=""
pass="yes"
printf "Welcome to Simon. I wil be flashing colors on the screen and you'll have to memorize it and repeat it back to me. Write the answers with either the first letter of the color or the full color name, with either no spaces or just one.\n"
...
while [[ "$pass" == "yes" ]]; do rand=$(shuf -i 1-4 -n 1)
    ...
    sleep 1
    clear
    #elifs are boring, let's try cases now. According to SO cases are slightly faster, so maybe I'll use those exclusively for >1 comparisions from now on.
    ...
    while [[ "$pass" == "yes" ]]; do
        ...
        inp=${inp//blue/b}; inp=${inp//red/r}; inp=${inp//yellow/y}; inp=${inp//green/g} #To allow users to mix single letter and full word answers in the same input line. There has to be a better way of formatting this..
        case $inp in
            "$curr"|"$currFirst")
